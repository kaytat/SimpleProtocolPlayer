 ...
 * remote controls capable of displaying metadata, artwork and media
 * transport control buttons. A remote control client object is associated
 * with a media button event receiver. This event receiver must have been
...
public class RemoteControlClientCompat {
  ...
  public RemoteControlClientCompat(PendingIntent pendingIntent,
    ...
    try {
      ...
      Log.e(TAG, "Error creating new instance of " +
   ...
   * {@link android.media.RemoteControlClient#editMetadata(boolean)} to
   * create an instance of an editor, on which you set the metadata for the
   * RemoteControlClient instance. Once all the information has been set,
   * use {@link #apply()} to make it the new metadata that should be
  ...
  public class MetadataEditorCompat {
    ...
    private MetadataEditorCompat(Object actualMetadataEditor) {
      ...
      if (sHasRemoteControlAPIs) {
        ...
        try {
          mPutStringMethod =
              metadataEditorClass.getMethod("putString",
                  int.class, String.class);
          mPutBitmapMethod =
              metadataEditorClass.getMethod("putBitmap",
     ...
     * @param value The text for the given key, or {@code null} to
     *              signify there is no valid information for the field.
     ...
     * control.
     *
     * @param key    the identifier of the bitmap to set. The only valid
     *               value is {@link #METADATA_KEY_ARTWORK}
     ...
     * Note that none of the information added after {@link #apply()} has
     * been called,
     * will be displayed.
     *
     * @param key   the identifier of a the metadata field to set. Valid
   ...
   *
   * @param startEmpty Set to false if you want the MetadataEditor to
   *                   contain the metadata that
   *                   was previously applied to the RemoteControlClient,
   *                   or true if it is to be created empty.
  ...
  public MetadataEditorCompat editMetadata(boolean startEmpty) {
    ...
    if (sHasRemoteControlAPIs) {
      try {
        metadataEditor = sRCCEditMetadataMethod
            .invoke(mActualRemoteControlClient,
                startEmpty);
      } catch (Exception e) {
        throw new RuntimeException(e);
      }
  ...
  public void setPlaybackState(int state) {
    if (sHasRemoteControlAPIs) {
      try {
        sRCCSetPlayStateMethod
            .invoke(mActualRemoteControlClient, state);
      } catch (Exception e) {
        throw new RuntimeException(e);
      }
    }
  ...
  /**
   * Sets the flags for the media transport control buttons that this
   * client supports.
   *
